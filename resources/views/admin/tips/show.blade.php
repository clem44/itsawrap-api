...
<div class="profile-container">
    <a href="{{ route('admin.tips.index') }}" class="back-link">
        ...
        <div class="profile-header-content">
            <div class="profile-header-top">
                <div class="profile-avatar-section">
                    <div class="profile-avatar">
                        {{ strtoupper(substr((string) $tip->id, 0, 2)) }}
                    </div>
                    <div class="profile-info">
                        <h1 class="heading-serif text-3xl font-semibold">Tip #{{ $tip->id }}</h1>
                        <p class="profile-username">Order #{{ $tip->order?->number ?? '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="profile-stats">
                <div class="stat-card">
                    ...
                    <div class="stat-value">${{ number_format($tip->amount, 2) }}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="stat-value">{{ $tip->created_at->format('M d, Y H:i') }}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="stat-value">{{ $tip->order?->customer?->name ?: 'Walk-in' }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="section-card animate-in animate-delay-1">
        ...
        @if($tip->order)
            <div class="overflow-x-auto">
                <table class="tokens-table">
                    ...
                    <tbody>
                        @php
                            $statusColor = match($tip->order->status?->code ?? 'pending') {
                        ...
                        <tr>
                            <td>
                                <span class="token-name">#{{ $tip->order->number }}</span>
                            </td>
                            <td>
                                <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full" style="background: {{ $statusColor['bg'] }}; color: {{ $statusColor['text'] }};">
                                    {{ $tip->order->status?->name ?? 'Unknown' }}
                                </span>
                            </td>
                            <td>
                                <span class="token-meta font-semibold">${{ number_format($tip->order->total, 2) }}</span>
                            </td>
                            <td>
                                <span class="token-meta">{{ $tip->order->created_at->format('M d, Y H:i') }}</span>
                            </td>
                            <td>
                                <a href="{{ route('admin.orders.show', $tip->order) }}" class="btn-revoke" style="text-decoration: none;">View</a>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        @else
            <div class="empty-state">
                ...
                <p class="empty-state-text">This tip is not linked to an order.</p>
            </div>
        @endif
    </div>
</div>
