...
class OrderController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = Order::with(['customer', 'status', 'delivery.deliveryWindow', 'participants', 'orderItems.participant', 'orderItems.item', 'orderItems.orderItemOptions.optionValue.option'])
            ->leftJoinSub($paymentSummarySub, 'payment_summary', function ($join) {
                $join->on('orders.id', '=', 'payment_summary.order_id');
            })
            ->leftJoin('payments as payment_latest', 'payment_latest.id', '=', 'payment_summary.latest_payment_id')
    ...
    public function store(Request $request, RewardService $rewards): JsonResponse
    {
        ...
        return response()->json(
            $order->load(['customer', 'status', 'delivery.deliveryWindow', 'participants', 'orderItems.participant', 'orderItems.item', 'orderItems.orderItemOptions.optionValue.option']),
            201
        );
    }
    ...
    {
        return response()->json(
            $order->load(['customer', 'status', 'session', 'delivery.deliveryWindow', 'participants', 'orderItems.participant', 'orderItems.item', 'orderItems.orderItemOptions.optionValue.option', 'payments', 'tips'])
        );
    }
    ...
    public function update(Request $request, Order $order, RewardService $rewards, CustomerPushNotifier $customerPushNotifier): JsonResponse
    {
        ...
        return response()->json($order->load(['customer', 'status', 'delivery.deliveryWindow', 'participants', 'orderItems.participant', 'orderItems.item', 'orderItems.orderItemOptions.optionValue.option']));
    }
