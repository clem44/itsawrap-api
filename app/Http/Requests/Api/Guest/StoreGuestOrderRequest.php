        ...
        return [
            function (Validator $validator): void {
                if (strlen((string) $this->getContent()) > 65535) {
                    $validator->errors()->add('payload', 'The guest order payload is too large.');
                }
                ...
                foreach ($this->input('items', []) as $index => $item) {
                    ...
                    if ($itemModel === null || ! $itemModel->active) {
                        $validator->errors()->add("items.$index.item_id", 'The selected item is not available for guest ordering.');
                    }
                ...
                foreach ($this->input('bundles', []) as $index => $bundle) {
                    ...
                    if ($bundleModel === null) {
                        $validator->errors()->add("bundles.$index.bundle_id", 'The selected bundle is not available for guest ordering.');
                ...
                if ($totalQuantity > 100) {
                    $validator->errors()->add('items', 'Guest orders may not contain more than 100 total item quantities.');
                }

                if ($totalOptions > 200) {
                    $validator->errors()->add('items', 'Guest orders may not contain more than 200 option selections.');
                }
            },
        ];
    }
